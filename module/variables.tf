...
variable "records" {
  ...
  validation {
    condition = var.zone != null || alltrue([
      for record in values(var.records) : try(trimspace(record.zone_id) != "", false)
    ])
    error_message = "Each Route 53 record requires zone_id when no zone is defined."
  }
}
